...
If you map the same continuous variable to both x and y aesthetics in a scatterplot, you get a 
straight ___ line. (Choose between “vertical”, “horizontal”, or “diagonal”.) 
...
3. In your own words, define the functionality of the following: 
...
4. Reference the help file of the `mtcars` data set. Then fill in the following `...` to make a scatterplot of cars' `miles per gallon` on the x-axis and their `weight (1000lbs)` below.
```{webr-r}
ggplot(mtcars) + 
  aes(x = ... , y = ...) + 
  geom_...()
...
5. Connect the set of variables with a geom that will create an appropriate visualization.
